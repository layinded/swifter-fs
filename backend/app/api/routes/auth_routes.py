...
from app.core.security.password_security import generate_password_reset_token, verify_password_reset_token, \
    get_password_hash
...
from app.core.security.refresh_token_service import revoke_all_tokens
from app.core.utils.email import generate_reset_password_email, send_email
from app.crud import crud_user
from app.models.auth import Token, TokenRefreshRequest  # ✅ Fix missing import
...
def login_user(session: SessionDep, form_data: OAuth2PasswordRequestForm = Depends()) -> Token:
    """OAuth2 login that returns an access token and refresh token."""
    existing_user = crud_user.authenticate(session=session, email=form_data.username, password=form_data.password)
    ...
    access_token = create_access_token(existing_user.email, expires_delta=access_token_expires)
    refresh_token = create_refresh_token(session, existing_user.email, expires_delta=refresh_token_expires)
...
def refresh_access_token(session: SessionDep, request: TokenRefreshRequest) -> Token:
    ...
    new_access_token = create_access_token(email, expires_delta=access_token_expires, auth_provider=auth_provider)

    # ✅ Generate a new refresh token and update the DB
    new_refresh_token = create_refresh_token(session, email, expires_delta=refresh_token_expires,
...
@router.post("/register", response_model=user.UserPublic, operation_id="register_new_user")
...
@router.patch("/password/update", response_model=common.Message, operation_id="change_password")
def update_password(session: SessionDep, body: user.UpdatePassword, current_user: CurrentUser) -> Any:
    """Update the password for the currently logged-in user."""
    return user_service.update_password(session, current_user.id, body.current_password, body.new_password)


# 🔹 ✅ Delete current user account
@router.delete("/profile/delete", response_model=common.Message, operation_id="delete_current_user")
...
def recover_password(email: str, session: SessionDep) -> common.Message:
    ...
    if existing_user.auth_provider != "local":
        raise HTTPException(status_code=400, detail="Password reset is only available for local users.")

    password_reset_token = generate_password_reset_token(email=email)
    email_data = generate_reset_password_email(email_to=existing_user.email, email=email, token=password_reset_token)

    send_email(email_to=existing_user.email, subject=email_data.subject, html_content=email_data.html_content)
...
def reset_password(session: SessionDep, body: user.NewPassword) -> common.Message:
    ...
    return common.Message(message="Password reset successful. You have been logged out.")
